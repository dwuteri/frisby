...
// XML and XPath wrapper object
function xml(xml_string) {
  this.doc = new dom().parseFromString(xml_string)
  return this
}

xml.prototype.select = function(xpath_string) {
  return xpath.select(xpath_string, this.doc)
}

xml.prototype.select1 = function(xpath_string) {
  return xpath.select1(xpath_string, this.doc)
}
...
xml.prototype.forEachNode = function(xpath, fn) {
  _.each(this.select(xpath), function(node) {
...
Frisby.prototype.postXML = function (uri, xml_string) {
  return this.post(uri, {}, {body: xml_string});
};
...
// Helper to check parse HTTP response body as XML and check that XPath queries TODO: finish this comment
Frisby.prototype.expectXPath = function() {
  for (var i = 0; i < arguments.length; i++) {
    if (typeof arguments[i] !== 'string')
      throw "Frisby.expectXML(): only strings are accepted as arguments";
  }
  ...
  this.current.expects.push(function() {
    var body = new xml(self.current.response.body);
    _.each(qrs, function(qr) {
      expect(body).toContainXPath(qr);
    });
  });
  return this;
};
...
// Debugging helper to inspect XML response body content recieved from server
Frisby.prototype.inspectXML = function(xpath) {
  if(typeof xpath === "undefined") {
    xpath = '/'
  }
  this.afterXML(function(xml) {
    console.log("xml:[" + String(xml.select(xpath)) + "]");
  });
  return this;
};
...
// Callback function to run after test is completed
// Helper to also automatically convert response body to XML
Frisby.prototype.afterXML = function(cb) {
  var self = this;
  this.current.after.push(function() {
    cb.call(this, new xml(self.current.response.body));
  });
  return this;
};
...
beforeEach(function() {
  this.addMatchers({
    toContainXPath: function(xpath) {
      this.message = function() {
        return "Actual XML did not have the XPath '" + xpath + "'";
      }
      return this.actual.select(xpath).length > 0;
    }
  });
});
